...
<template>
    <nav class="flex items-center page-content justify-between h-3.5rem md:h-16 relative">
        ...
        <ul class="gap-6 text-sm hidden md:flex text-primary font-semibold">
          <li v-for="link in links" :key="link.path" class="cursor-pointer py-1.5 px-4 rounded-[8rem]" :class="{'bg-[#F6F6F7]': pageContext.urlPathname === link.path}"> {{ link.label }}</li>
        </ul>
