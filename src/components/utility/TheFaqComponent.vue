...
<template>
    <div class="flex flex-col gap-6">
        <div v-for="(faq, i) in faqs" :key="i" class="">
            <div class="md:px-6 border-[#BCDCDC]" :class="{ 'border-b': i + 1 !== faqs.length }">
                ...
                <p class="text-blue-200 mt-6 max-h-0 overflow-hidden transition-all  duration-300 select-none"
                    :class="{ '!max-h-100rem pb-6 md:pb-8': faq.isOpen, '!duration-1400': faq.isOpen }">
                    {{
                        faq.text }} </p>
            </div>
        </div>
