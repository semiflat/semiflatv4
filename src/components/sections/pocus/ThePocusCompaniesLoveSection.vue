<script setup lang='ts'>
import Ngrok from '~/assets/pocus/companies/ngrok.svg?component'
import Openphone from '~/assets/pocus/companies/openphone.svg?component'
import Clockwise from '~/assets/pocus/companies/clockwise.svg?component'
import Linear from '~/assets/pocus/companies/linear.svg?component'
import Quicknode from '~/assets/pocus/companies/quicknode.svg?component'
import Miro from '~/assets/pocus/companies/miro.svg?component'
import Webflow from '~/assets/pocus/companies/webflow.svg?component'
import LeftBg from '~/assets/pocus/left-cubes.svg?component'
import RightBg from '~/assets/pocus/right-cubes.svg?component'

</script>

<template>
    <div class="page-content mt-16 md:mt-40">
        <div
            class="bg-violet-gradient py-8 px-6 text-white rounded-3 relative lg:px-[7.125rem] lg:py-20 max-w-[57.125rem] mx-auto">
            <div class="absolute w-full h-full top-0 left-0 z-1 card-mask">
                <component :is="RightBg" class="absolute top-0 right-0 lg:w-[150px] lg:h-[150px]  lg:top-4" />
                <component :is="LeftBg" class="absolute bottom-0 lg:w-[216px] lg:h-[216px] lg:" />
            </div>

            <h3 class="text-center mb-8">These companies love Pocus:</h3>
            <div class="lg:hidden">
                <div class="grid grid-cols-2 md:grid-cols-3 items-center gap-y-8">
                    <!-- <component :is="logo" v-for="logo in companies[]" :key="logo" class="max-w-full h-[2.375rem] mx-auto" /> -->
                    <div class="h-[38px]">
                        <component :is="Ngrok" class="max-w-full h-[28px] lg:h-[30px] mx-auto" />
                    </div>
                    <div class="h-[38px]">
                        <component :is="Openphone" class="max-w-full h-[18px] lg:h-[24px] mx-auto" />
                    </div>
                    <div class="h-[38px]">
                        <component :is="Clockwise" class="max-w-full h-[20px] lg:h-[28px] mx-auto" />
                    </div>
                    <div class="h-[38px]">
                        <component :is="Linear" class="max-w-full h-[24px] lg:h-[28px] mx-auto" />
                    </div>
                    <div class="h-[38px]">
                        <component :is="Quicknode" class="max-w-full h-[26px] lg:h-[32px] mx-auto" />
                    </div>
                    <div class="h-[38px]">
                        <component :is="Miro" class="max-w-full h-[22px] lg:h-[26px] mx-auto" />
                    </div>
                </div>
                <div class="h-[38px] mt-8">
                    <component :is="Webflow" class="max-w-full h-[24px] lg:h-[28px] mx-auto " />
                </div>
            </div>
            <div class="hidden lg:block">
                <div class="flex gap-12 justify-center mb-8">
                    <div class="h-[40px]">
                        <component :is="Ngrok" class="lg:h-[38px] mx-auto -mt-1" />
                    </div>
                    <div class="h-[40px]">
                        <component :is="Openphone" class="max-w-full lg:h-[24px] mx-auto" />
                    </div>
                    <div class="h-[40px]">
                        <component :is="Clockwise" class="max-w-full  lg:h-[28px] mx-auto" />
                    </div>
                    <div class="h-[40px]">
                        <component :is="Linear" class="max-w-full lg:h-[28px] mx-auto" />
                    </div>
                </div>
                <div class="flex gap-12 justify-center">
                    <div class="h-[40px]">
                        <component :is="Quicknode" class="lg:h-[38px] mx-auto -mt-1" />
                    </div>
                    <div class="h-[40px]">
                        <component :is="Miro" class="max-w-full lg:h-[24px] mx-auto" />
                    </div>
                    <div class="h-[40px]">
                        <component :is="Webflow" class="max-w-full  lg:h-[28px] mx-auto" />
                    </div>
                </div>

            </div>
        </div>
    </div>
</template>

<style scoped>
.bg-violet-gradient {
    background: radial-gradient(57.03% 76.98% at 100% 0%, rgba(255, 255, 255, 0.8) 0%, rgba(255, 255, 255, 0) 100%), linear-gradient(0deg, rgba(0, 0, 0, 0.48), rgba(0, 0, 0, 0.48)), #481B97;
    background-blend-mode: overlay, normal, normal;

    box-shadow: 0px 4px 12px -2px rgba(17, 24, 28, 0.12), 0px 1px 2px rgba(17, 24, 28, 0.16);
}


.card-mask {
    -webkit-mask-image: radial-gradient(50% 50% at 50% 50%, #D9D9D9 55.46%, rgba(217, 217, 217, 0) 100%);
    mask-image: radial-gradient(50% 50% at 50% 50%, #D9D9D9 55.46%, rgba(217, 217, 217, 0) 100%)
}

@media (min-width: 1366px) {
    .card-mask {
        -webkit-mask-image: radial-gradient(63.02% 50% at 50% 50%, #D9D9D9 55.46%, rgba(217, 217, 217, 0) 100%);
        mask-image: radial-gradient(63.02% 50% at 50% 50%, #D9D9D9 55.46%, rgba(217, 217, 217, 0) 100%);
    }
}
</style>